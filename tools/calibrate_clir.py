#!/usr/bin/env python3
"""Fit conservative CLIR action-uplift bounds on development records."""
...
from lgagent.clir import InterventionRecord, fit_clir_calibration


def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Fit CLIR on an independent development split. Other splits in "
            "the input file are ignored, not used for calibration."
        )
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    artifact = fit_clir_calibration(
        _read_split(args.input, "dev"),
        alpha=args.alpha,
        group_field=args.group_field,
        min_calibration_size=args.min_calibration_size,
        min_group_size=args.min_group_size,
    )
    _atomic_json(args.output, artifact.as_dict())
    print(
        json.dumps(
            {
                ...
                "actions": {
                    ...
                    for action, estimate in artifact.global_estimates.items()
                },
